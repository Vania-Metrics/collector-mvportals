// colecteur-mvportals — a VaniaMetrics-<Name>-<v>.jar in build/libs/
//
...
version = Regex("""VALUE = "([^"]+)"""").find(versionSource.readText())?.groupValues?.get(1)
    ?: error("cannot read version in $versionSource")
...
// listing addresses modules by their plugin name. One source, the one Bukkit
// displays.
