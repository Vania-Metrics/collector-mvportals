...
VaniaMetrics collector for mvportals. One module = one jar, loaded by the platform if and only if the core is present.

## Build

```sh
./gradlew build                                # build/libs/vania-metrics-collector-mvportals-<v>.jar
...
- `gradle/libs.versions.toml` — paper-api, velocity-api, and the targeted plugins (Modrinth's Maven repo), at compile time only.
